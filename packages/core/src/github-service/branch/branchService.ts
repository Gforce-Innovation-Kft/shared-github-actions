/**
 * Branch domain of the GitHub service: the {@link BranchService} port and its
 * single Octokit-backed implementation, colocated. Comparing refs, resolving a
 * head SHA, moving a ref, and server-side merges.
 *
 * `@octokit/rest` is touched only by {@link OctokitBranchService}; orchestrators
 * depend on the {@link BranchService} interface. The constructor is injectable
 * (tests pass a fake `Octokit`) and the singleton statics share the one
 * {@link GitHubClient} so a single authenticated client backs every call.
 */
